...
## Results
  ...
  Age, Gender, State Trait Anxiety, Pain Catastrophizing, dispositional mindfulness and hormonal responses were used to predict a person’s overall pain level in the postperative pain of wisdom tooth extraction.  The data were screened for assumptions, and `r sum(total.out)` participants were eliminated due to high Cook’s and Leverage values.  Linearity, normality, multicollinearity, homogeneity, and homoscedasticity were all met.

  Age and sex were entered first into a hierarchical regression to control for demographic differences in the level of postoperative pain.  Overall, this model was significant, indicating that demographics predict the level of pain patients feel after surgery (F(2, 140) = 6.267, p < .005, R2adj = .069). The model also qualified to have an acceptable level of goodness of fit (AIC = 494.4865). Out of the variables investigated, only *age* was significant with a negative effect size (β = -0.29, t(140) = 3.53, p < .001). The result assumes that younger patients experience less postoperative pain.
  Next, State Trait Anxiety, Pain Catastrophizing, dispositional mindfulness and hormonal responses were added in a second step to examine their predictive value after controlling for demographic variables. During the data screening process the two variables measuring cortisol levels showed high correlational values (r =-0.89). In the end, cortisol_serum was dropped since cortisol_saliva had a better correlation with the dependent variable.  The addition of these variables showed a significant model that explained more variance in our dependent variable (F(6, 136) = 14.72, p < .0001, R2adj = .367). The second model also showed an acceptable level of goodness (AIC = `r AIC(model2)`). The model only presented two significant predictors. Cortisol levels measured from saliva (β = 0.42, t(136) = 4.74, p < .00`1) showed stronger predictive value than Pain catastrophizing (β = 0.27, t(136) = 2.98, p < .001). This result assumes that those, who have high cortisol levels and magnify the threat value of a pain stimulus will experience stronger postoperative pains.
  Adding predictors to the second variable showed significant changes and resulted in an improved model ΔF(4, 136) = 17.475, p < .001, ΔR2adj= .298). We can observe that the second model explained more variance in our dependent variable. Interestingly, age was not identified as a significant predictor. 

## Discussion

In our effort to investigate postoperative pain of wisdom tooth surgery we learned that failure to inhibit thought of fear and anxiety results in a greater magnitude of postoperative pain. Giving patients distractors while they are in the waiting room might help with their levels of pain experienced.
